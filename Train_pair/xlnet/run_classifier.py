...
flags.DEFINE_float("predict_threshold", default=0.5,
      help="Threshold for binary prediction.")
flags.DEFINE_string("eval_split", default="test", help="could be dev or test")
flags.DEFINE_integer("eval_batch_size", default=512,
      help="batch size for evaluation")
flags.DEFINE_integer("predict_batch_size", default=512,
...
class MyProcessor(DataProcessor):
  ...
  def get_test_examples(self, data_dir):
    ...
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir,
            "pair_siamese_bert_base_cased_epoch2_k_100_positive.txt")), "test")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      text_a = preprocess_text(line[0], lower=FLAGS.uncased)
      text_b = preprocess_text(line[1], lower=FLAGS.uncased)
      if set_type == "test":
        label = "0"
      else:
  ...
  if FLAGS.do_predict:
    ...
    with tf.gfile.Open(os.path.join(predict_dir, "{}.tsv".format(
      ...
      for pred_cnt, result in enumerate(estimator.predict(
        ...
        probabilities_results.append("\t".join(
    ...
    output_predict_file = os.path.join(predict_dir, "test_results.tsv")
    with tf.gfile.GFile(output_predict_file, "w") as writer:
      tf.logging.info("***** Predict results *****")
      for prediction in probabilities_results:
