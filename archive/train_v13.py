...
from project_dataset_binary import ProjectDataset
...
parser.add_argument('--model_version_string', type=str, default="v13_tier2")
parser.add_argument('--log_every', type=int, default=500)

args = parser.parse_known_args()[0]
tb_writer = SummaryWriter(log_dir='tb_logs', comment=args.model_version_string)
...
def eval_model(epoch, model, loader):
    ...
    for i, batch in tqdm(enumerate(loader)):
        start = time.time()
        with torch.no_grad():
            loss, acc = batch_eval(batch, model)
...
def train_model(epoch, model, optimizer, lr_scheduler, loader, test_loader):
    ...
    for i, batch in tqdm(enumerate(loader)):
        ...
        if (GLOBAL_STEP) % args.log_every == 0:
            ...
            tb_writer.add_scalar('Training/training_loss', loss_meter.avg, GLOBAL_STEP)
            tb_writer.add_scalar('Training/training_acc', acc_meter.avg, GLOBAL_STEP)
